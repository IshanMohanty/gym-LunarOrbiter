...
#
# python gym/envs/box2d/lunar_orbiter.py
#
# To play yourself, run:
#
# python examples/agents/keyboard_agent.py LunarOrbiter-v2
#
...
LANDER_POLY = [
    (-15, +40), (-15, 0), (-15, -40),      # define the shape of body
    (+15, -40), (+15, 0), (+15, +40), (0, 60)       # define the shape of body
]
"""##################### define the fire shape ####################"""
FIRE_POLY=[(-6, -10), (-0.5, -20), (0.5, -20), (6, -10)]
MAX_SMOKE_LIFETIME = 2 * FPS
"""##################### define the fire shape ####################"""
LEG_AWAY = 10                              # the horizontal distance between leg and body
LEG_DOWN = 50                              # the vertical distance between leg and body
LEG_W, LEG_H = 4, 13                        # the shape of legs
...
class LunarOrbiter(gym.Env, EzPickle):
    metadata = {
    ...
    def reset(self):
        ...
        # terrain
        CHUNKS = 21                                                       # the number of vertex on surface
        ...
        # self.helipad_x1 = chunk_x[CHUNKS // 2 - 1]                        # define the x-position of left flag
        # self.helipad_x2 = chunk_x[CHUNKS // 2 + 1]                        # define the y-position of right flag
        ...
        # height[CHUNKS // 2 + 1] = self.helipad_y
        # height[CHUNKS // 2 + 2] = self.helipad_y             # take the average of 3-neighbor vertex, smooth the slope
        smooth_y = [0.33 * (height[i - 1] + height[i + 0] + height[i + 1]) for i in range(CHUNKS)]
        # smooth_y = [1 * (0*height[i - 1] + height[i + 0] + 0*height[i + 1]) for i in range(CHUNKS)]

        """########################### change the position of flag ##############################"""
        index = np.random.randint(1, CHUNKS-2)
        self.helipad_one_x = chunk_x[index]
        self.helipad_one_y = smooth_y[index]
        smooth_y[index+1] = self.helipad_one_y
        # smooth_y[index+2] = smooth_y[index+2] * 0.1
        """########################### change the position of flag ##############################"""
        ...
        # self.earth = self.world.CreateStaticBody(center=(1.5, 1.5))
        # self.earth.color1 = (0.0, 0.40, 0.80)
        # self.earth.color2 = (0.0, 0.40, 0.80)
        self._create_clouds()
        ...
        self.lander = self.world.CreateDynamicBody(
            ...
            # position=(VIEWPORT_W / SCALE / 2, initial_y),               # but no idea how to change it
        ...
        """############################### Draw Fire and Smoke ##############################"""
        for i in [-1, +1]:
            self.fire = self.world.CreateDynamicBody(
                position=(VIEWPORT_W / SCALE / 2, initial_y + 17 / SCALE),
                angle=0.0,
                fixtures=fixtureDef(
                    shape=polygonShape(vertices=[(x / SCALE, y / SCALE) for x, y in FIRE_POLY]),
                    categoryBits=0x0010,  # the category the lander belongs to
                    maskBits=0x001)  # the category the lander with collide with
            )
            self.fire.color1 = (0.7, 0.25, 0.25)
            self.fire.color2 = (0.7, 0.25, 0.25)
        ...
        self.fire.joint = self.world.CreateJoint(rjd)
        """############################### Draw Fire and Smoke ##############################"""
        self.legs = []
        for i in [-1, +1]:
        ...
        # self.drawlist = [self.lander] + self.legs + [self.fire]
        # self.drawlist = [self.lander]            # draw the graph or not: no drawing does not mean it is not existing
    ...
    """########################### Create Stars ##############################"""
    def _create_cloud(self, x_range, y_range, y_variance):
        self.cloud_poly = []
        ...
        x_step = np.linspace(initial_x, initial_x+np.random.uniform(1, 6), 200)
    ...
    def _create_clouds(self):
        self.clouds = []
        for i in range(10):
            self.clouds.append(self._create_cloud([-0.2, 0.4], [0.65, 0.7], 0.5))
            self.clouds.append(self._create_cloud([0.7, 0.8], [0.75, 0.8], 0.5))
            self.clouds.append(self._create_cloud([1.0, -0.8], [-0.35, 0.4], 0.5))
            self.clouds.append(self._create_cloud([-1.0, -0.8], [0.75, 1.2], 0.5))
            self.clouds.append(self._create_cloud([0.7, -0.8], [-0.65, 0.3], 0.5))
    ...
    def step(self, action):                                                                # physical engine
        ...
        state = [
            ...
            self.lander.angle,
            20.0 * self.lander.angularVelocity / FPS,                                # angular velocity
    ...
    def render(self, mode='human'):                                                 # graphic engine
        ...
        #     self.viewer.draw_polygon(x, color=(1.0, 1.0, 0.0))
        ...
        # draw "earth"
        self.viewer.draw_circle(center=(0, -10), radius=10, color=(0.0, 0.40, 0.80))  # the center of circle will not move
        """"################################## create stars ########################################"""
        ...
        #     flagy2 = flagy1 + 70 / SCALE                                                # define the length of pole
        #     self.viewer.draw_polyline([(x, flagy1), (x, flagy2)], color=(1, 1, 1))      # deine the shape of flag
        #     self.viewer.draw_polygon([(x, flagy2), (x, flagy2 - 10 / SCALE), (x + 25 / SCALE, flagy2 - 5 / SCALE)],
        #                              color=(0.8, 0.8, 0))                               # define the shape of flag
            # self.viewer.draw_polygon([(x, flagy2), (x, flagy2 - 20 / SCALE), (x + 40 / SCALE, flagy2-20/SCALE), (x+40/SCALE, flagy2)],
            #                          color=(178/255, 34/255, 34/255))

        # draw 1 flag
        # for x in [self.helipad_x1, self.helipad_x1]:
        #     flagy1 = self.helipad_y
        #     flagy2 = flagy1 + 70 / SCALE                                                # define the length of pole
        #     self.viewer.draw_polyline([(x, flagy1), (x, flagy2)], color=(1, 1, 1))      # deine the shape of flag
            # self.viewer.draw_polygon([(x, flagy2), (x, flagy2 - 10 / SCALE), (x + 25 / SCALE, flagy2 - 5 / SCALE)],
            #                          color=(0.8, 0.8, 0))                               # define the shape of flag
            # self.viewer.draw_polygon([(x, flagy2), (x, flagy2 - 25 / SCALE), (x + 40 / SCALE, flagy2-25/SCALE), (x+40/SCALE, flagy2)],
            #                          color=(178/255, 34/255, 34/255))
...
class LunarOrbiterContinuous(LunarOrbiter):
    continuous = True
...
if __name__ == '__main__':
    demo_heuristic_lander(LunarOrbiter(), render=True)
